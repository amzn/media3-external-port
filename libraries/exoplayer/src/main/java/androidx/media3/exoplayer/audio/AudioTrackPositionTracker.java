...
/* package */ final class AudioTrackPositionTracker {
  ...
  private static final String TAG = AudioTrackPositionTracker.class.getSimpleName();  // AMZN_CHANGE_ONELINE
  ...
  public AudioTrackPositionTracker(Listener listener, 
    ...
    if (Util.SDK_INT >= 18) {
      try {
        ...
        Log.w(TAG, "Some amazon legacy devices throw unexpected errors", e);    // AMZN_CHANGE_ONELINE
      }
    }
    ...
    if(applyDolbyPassThroughQuirk) {
      ...
      if (VDBG) {
        log.v("getCurrentPositionUs : hasTimestamp: positionUs = " + positionUs + 
              ...
              " elapsedSinceTiemstampUs = " + elapsedSinceTimestampUs  + 
              " systemTimeUs = " + systemTimeUs + 
              " timestampSystemUs = " + audioTimestampPoller.getTimestampSystemTimeUs());
      }
      // AMZN_CHANGE_END
    } else {
  ...
  private boolean forceHasPendingData() {
    ...
    boolean hasPendingDataQuirk = AmazonQuirks.isLatencyQuirkEnabled()
            && ( checkNotNull(audioTrack).getPlayState() == AudioTrack.PLAYSTATE_PLAYING )
  ...
  private void updateRawPlaybackHeadPosition(long currentTimeMs) {
    ...
    if (this.rawPlaybackHeadPosition > rawPlaybackHeadPosition
          && this.rawPlaybackHeadPosition > 0x7FFFFFFFL
          && (this.rawPlaybackHeadPosition - rawPlaybackHeadPosition >= 0x7FFFFFFFL )){
